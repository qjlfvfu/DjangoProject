...
def profile_view(request):
    ...
    return render(
        request,
        "botblock/profile.html",
        {"title": _("Мой профиль"), "user": request.user, "products": products},
    )
...
def profile_edit(request):
    ...
    return render(
        request,
        "botblock/profile_edit.html",
        {"form": form, "title": _("Редактирование профиля")},
    )
        ...
        if form.is_valid():
            ...
            if user.check_password(old_password):
                ...
                update_session_auth_hash(
                    request, user
                )  # Важно! Оставляет пользователя в системе
